...
이 절은 **미배포 소스의 선택 기능**입니다. 위 v0.4.0 설치본에 제공된다고 가정하지
마세요. 이번 개발 작업에서 기존 설치나 운영 프로필을 자동 변경하지 않았습니다.

엄격 모드는 새 쓰기를 준비·실행 절차로만 받습니다. 일반 제품 CLI는 이미 이 절차를
사용하므로 사용자가 직접 작업 번호를 만들 필요가 없습니다. MCP 클라이언트는 위의
`workspace_prepare` → `workspace_execute` 흐름을 사용해야 합니다.

별도로 승인된 시험 폴더에서 새 소스를 실행하는 경우의 설정 예시입니다.
아래 명령은 실제 프로필을 변경하므로, 운영 폴더에 확인 없이 실행하지 마세요.

```bash
yeoul configure ./test-workspace --mode discuss --write-policy prepared_only
yeoul doctor --workspace ./test-workspace
```

`doctor`의 `write_policy`는 저장된 설정을 보여줍니다. 이미 연결된 서버의 적용 상태를
증명하지 않습니다. 적용하려면 별도 승인 후 서버를 다시 연결해야 합니다.
기존 설정은 `compatible`이며 자동 전환되지 않습니다. 설정 변경은 이전 프로필을
보존하고 기존 검증 명령 실행 승인을 취소합니다.

| 응답 | 의미와 안전한 다음 행동 |
|---|---|
| `preparation_required` | 실행이 시작되지 않았습니다. 현재 내용을 검토하고 새 작업을 준비하세요. |
| `stale_precondition` | 준비 후 대상이 바뀌었습니다. 변경을 검토한 뒤 새 작업을 준비하세요. |
| `reconciliation_required` | 이전 실행이 불확실합니다. 새 번호로 우회하지 말고 아래 중단·복구 절차를 따르세요. |

실행 전의 구형 작업은 그대로 보존하되 엄격 모드에서 실행하지 않습니다. 완료된 작업은
현재 권한을 다시 확인한 뒤 과거 응답을 반환합니다. 이는 지금 상태에서도 성공했다는
뜻이 아닙니다. 이 모드는 작업자의 직접 파일 편집, 임의 셸 실행이나 관리자 설정 변경을
OS 수준으로 막지 않습니다. [정확한 보호 범위](PREPARED_FRESHNESS.md)를 확인하세요.
...
미배포 개발판의 조회는 작업 기록 하나가 손상돼도 나머지 작업을 표시합니다.
`evidence_unreadable`이면 원본을 보존하고 누락·손상된 증거부터 확인해야 합니다.
`interrupted_operation`이면 남은 프로세스와 실제 업무 변경을 조사해야 합니다.
`orphaned_pending_receipt`는 실행 중 기록은 있지만 활성 작업 표시가 없다는 뜻입니다.
누락된 기록부터 조사하고 새 작업 ID로 우회하지 마세요.
`no_active_interruption_detected`는 진행 중 중단 기록을 찾지 못했다는 뜻일 뿐,
업무 성공이나 재시도 허가가 아닙니다. 모든 진단의 `retry_authorized`는 `false`입니다.
손상된 기록을 읽을 수 있게 보여주는 것이 실행·중단 해제의 검사를 완화하지는 않습니다.
설정 파일 자체가 유효하지 않으면 먼저 올바른 작업 폴더와 설정을 확인해야 합니다.
